<template>
			  ...
			  <div v-for="(item,index) in nameArray" class="select">
			   ...
			   <el-form-item :label="item.label" :prop="item.id">
			 	 <el-radio-group v-model="form[item.id]" fill="#4877E7"  @change="handEdit(item)">
			 	 	<!--<el-radio-button style="display: none;"></el-radio-button>-->
				    <el-radio-button :label="$data[item.id]"></el-radio-button>
				    <!--<el-radio-button style="display: none;"></el-radio-button>-->
	...
	<!--点击每个餐桌选项的编辑按钮-->
	<select-edit v-if="showSelectEdit"  @my-add="show" :name="selectName" @my-cancle="show2"></select-edit>
	</div>
</template>
<script>
...
//		            { type: number, message: '请输入数字' }
		...
		created() {
			...
			this.TABLE_TYPE=this.form.TABLE_TYPE=this.id.seat_shape||"方桌";
